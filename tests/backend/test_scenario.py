...
class TestScenario:
    ...
    def test_scenario_1_main(self):
        ...
        with qase.step("3. Wendy picks a VAs template to check and chat with it"):
            picked_va_template = list_of_va_templates[2]
        ...
        with qase.step("5. Wendy wants to edit skill"):
            va_components = user.get_va_components(va_name)
            for va_component in va_components['skills']:
                if va_component['component_type'] == 'Generative':
                    print(f'va_component = {va_component["component_id"]}')
                    print(va_component)

            # print(va_components['skills'][1])
...
#        with qase.step("0. Alex visits site: (alpha) (not login)_
#
