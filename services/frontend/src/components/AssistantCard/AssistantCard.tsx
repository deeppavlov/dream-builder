...
import { BotCardProps, TDistVisibility } from '../../types/types'
...
export const AssistantCard: FC<BotCardProps> = ({
  ...
  // Visibility & Deploy
  const visibility = bot?.visibility?.toLowerCase() as TDistVisibility
  const onModeration = bot?.publish_state === 'in_progress'
  const isPublished = visibility === 'public_template'
  const isDeployed = bot?.deployment?.state === 'UP'
  const isDeploying =
    !isDeployed && bot?.deployment?.state !== null && bot?.deployment !== null
  const visibilityName = !bot?.publish_state
    ? type === 'your' && visibility
    : onModeration
    ? 'On Moderation'
    : isPublished
    ? 'Public Template'
    : visibility
  const visibilityTagStatus = onModeration ? 'validating' : visibility
      ...
      <div className={s.body}>
        <div className={s.block}>
          ...
          <div className={s.dateAndVersion}>
            ...
            <SmallTag theme={visibilityTagStatus}>{visibilityName}</SmallTag>
          </div>
        </div>
